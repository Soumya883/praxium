...
import { db } from "@/db";
import { payments, students, users, communicationLogs, instituteExams } from "@/db/schema";
...
export async function sendExamResultWhatsApp(
  studentId: string, 
  examSubject: string, 
  ...
  try {
    const { authorized } = await checkRole(["ADMIN", "TEACHER"]);
    if (!authorized) {
      return { success: false, error: "Unauthorized access: Only teachers/admins can trigger exam alerts." };
    }
    ...
    let message = `*EXAM RESULT* 📊\n\nDear Guardian,\nThe results for the ${examSubject} test are out for ${studentProfile.studentName}.\n\n*Score:* ${marksObtained} / ${maxMarks}\n`;
    if (remarks) {
      message += `\n*Teacher Remarks:* ${remarks}\n`;
    }
    message += `\nPlease check the student portal for more details.`;
    ...
    console.error("[WHATSAPP EXAM RESULT DISPATCH ERROR]:", error);
    return { success: false, error: error.message || "An unexpected error occurred" };
  }
}
